...
def _validate_and_render(youtube: Resource, channel_data, playlist_id):
    ...
    return render_rss_feed(youtube, playlist_data, channel_data)


def create_rss_from_playlist(playlist_id: str):
    with youtube_service() as youtube:
        try:
            playlist_info = yt_playlist_info(youtube, playlist_id)
        except HttpError:
        ...
        return _validate_and_render(youtube, channel_data, playlist_id)
...
def create_rss_from_channel(yt_user: str, request_type: bool, channel_url: str):
    ...
    with youtube_service() as youtube:
        try:
            channel_data = yt_channels(youtube, yt_user, request_type, channel_url)
        ...
        return _validate_and_render(youtube, channel_data, channel_data.playlist_id)
